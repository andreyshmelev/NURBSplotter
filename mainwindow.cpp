...
void MainWindow::saveX1tofile(QString x1outputstr)
{
    if(!x1outputstr.isEmpty())
    {
        QString filename = QFileDialog::getSaveFileName(this, "DialogTitle", "x1.csv", "CSV files (.csv)", 0, 0); // getting the filename (full path)
        QFile data(filename);
        if(data.open(QFile::WriteOnly |QFile::Truncate))
        {
            QTextStream output(&data);
            output << QString(x1outputstr);
        }
    }
}

void MainWindow::saveY1tofile(QString y1outputstr)
{
    if(!y1outputstr.isEmpty())
    {
        QString filename = QFileDialog::getSaveFileName(this, "DialogTitle", "y1.csv", "CSV files (.csv)", 0, 0); // getting the filename (full path)
        QFile data(filename);
        if(data.open(QFile::WriteOnly |QFile::Truncate))
        {
            QTextStream output(&data);
            output << QString(y1outputstr);
        }
    }
}
    ...
    ui(new Ui::MainWindow)
{
    ...
    //  saveX1tofile(x1outputstr);


    //  saveY1tofile(y1outputstr);
...
void MainWindow::setupQuadraticDemo(QCustomPlot *customPlot)
{
    ...
    customPlot->graph(0)->setData(x1output, y1output);
    customPlot->graph(0)->setPen(QPen(Qt::red));
    customPlot->addGraph();
    //    customPlot->graph(0)->setData(x, y);

    customPlot->graph(1)->setData(x1input, y1input);
    customPlot->graph(1)->setPen(QPen(Qt::blue));

    customPlot->addGraph();

    customPlot->graph(2)->setData(x2output, y2output);
    customPlot->graph(2)->setPen(QPen(Qt::cyan));
    customPlot->addGraph();
    //    customPlot->graph(0)->setData(x, y);
    customPlot->addGraph();
    customPlot->graph(3)->setData(x2input, y2input);
...
double MainWindow::Nip(int i, int p, QVector<double> U, double u)
{
...
doublePoint MainWindow::RationalBSplinePoint(QVector<QPoint> Points,QVector<double> PointsWeight, int degree, QVector<double> KnotVector, double t)
{
    ...
    {
        double temp = Nip(i, degree, KnotVector, t)*PointsWeight[i];
        rationalWeight += temp;
    }

    //    qDebug() << rationalWeight << " rationalWeight";
    ...
    for (int i = 0; i < Points.length(); i++)
    {
        ...
        //        qDebug() << temp << " double temp";

        x += Points[i].x()*PointsWeight[i] * temp/rationalWeight;
        y += Points[i].y()*PointsWeight[i]  * temp / rationalWeight;

        //        qDebug() << x  << 'x'<< y  << 'y' << " temp x y ";

        //        qDebug() <<i<< "Points.length()";
    }
...
QVector<double> MainWindow::CalcualteKnotVector(int Degree,int ContolPointCount, bool IsUniform)
{
    QVector<double> vec;
    ...
    if (IsUniform)
    {
        ...
        for (int i = 1; i < m; i++)
        {
            ...
            {
                int dividend = m-1;
                out.append(", " + QString::number(i) + "/" + QString::number(dividend) );
            }
        }
    }
    else
    {
        ...
        for (int i = 1; i < m; i++)
        {
            ...
            else
            {
                ...
                out.append(", " + QString::number(dividend) + "/" + QString::number(divisor));
                //                qDebug() << dividend/divisor;
                vec.append(double(dividend/divisor));
            }
        }
    }


    //    qDebug() << out << "knot vector";
    return vec;
}

void MainWindow::on_Readx1FileButton_clicked()
{
    ...
    QString file1Name = QFileDialog::getOpenFileName(this,
             tr("Open data File"), "c:/", tr("txt Files (*.txt)"));
    ...
    QJsonArray jsonx1 = mainobj["x1"].toArray();
    QJsonArray jsony1 = mainobj["y1"].toArray();
    QJsonArray jsonw1 = mainobj["w1"].toArray();
    QJsonArray jsonx2 = mainobj["x2"].toArray();
    QJsonArray jsony2 = mainobj["y2"].toArray();
    QJsonArray jsonw2 = mainobj["w2"].toArray();


    QVector<double> x1indouble;
    QVector<double> x2indouble;
    QVector<double> y1indouble;
    QVector<double> y2indouble;
    QVector<double> w1indouble;
    QVector<double> w2indouble;


    for (int index = 0; index < jsonx1.count(); ++index) {
        x1indouble.append( jsonx1.at(index).toDouble());
    }



    for (int index = 0; index < jsonx2.count(); ++index) {
        x2indouble.append( jsonx2.at(index).toDouble());
    }



    for (int index = 0; index < jsony1.count(); ++index) {
        y1indouble.append( jsony1.at(index).toDouble());
    }



    for (int index = 0; index < jsony2.count(); ++index) {
        y2indouble.append( jsony2.at(index).toDouble());
    }




    for (int index = 0; index < jsonw1.count(); ++index) {
        w1indouble.append( jsonw1.at(index).toDouble());
    }


    for (int index = 0; index < jsonw2.count(); ++index) {
        w2indouble.append( jsonw2.at(index).toDouble());
    }
    ...
    for (int index = 0; index < x1indouble.length(); ++index) {

        Points1.append(QPoint(x1indouble[index],y1indouble[index]));
    }


    for (int index = 0; index < x2indouble.length(); ++index) {

        Points2.append(QPoint(x2indouble[index],y2indouble[index]));
    }


    Pointsweight1 = w1indouble;
    Pointsweight2 = w2indouble;

    for (int p = 0; p < Points1.length(); ++p) {
        x1input.append(Points1[p].x());
        y1input.append(Points1[p].y());
    }



    for (int p = 0; p < Points2.length(); ++p) {
        x2input.append(Points2[p].x());
        y2input.append(Points2[p].y());
    }

    int degree = ui->DegreeValue->value();

    KnotVector1 = CalcualteKnotVector(degree, Points1.length(),false);
    KnotVector2 = CalcualteKnotVector(degree, Points2.length(),false);

    doublePoint po;

    for (double part = 0; part <= 100; ++part) {

        po  = RationalBSplinePoint(Points1,Pointsweight1,degree,KnotVector1,part/100);
        y1output.append(po.y);
        x1output.append(po.x);
    }

    for (double part = 0; part <= 100; ++part) {

        po  = RationalBSplinePoint(Points2,Pointsweight2,degree,KnotVector2,part/100);
        y2output.append(po.y);
        x2output.append(po.x);
    }
